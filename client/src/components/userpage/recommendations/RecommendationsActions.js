      ...
      Promise.all(venues.map(venue => this.getVenueDetailsFromFoursquare(venue.id)))
        .then(detailedVenues => {
          const formattedVenues = detailedVenues.map(venue => ({
            ...
            opens: (venue.hours && venue.hours.timeframes[0].open[0].renderedTime.split('–')[0]) || '0000',
            closes: (venue.hours && venue.hours.timeframes[0].open[0].renderedTime.split('–')[1]) || '0000',
